...
import { ClaimsDocumentRef, Claims } from "../shared/claims";
...
export async function AssignCustomClaimsToUser(username: string, claims: Claims): Promise<void> {
...
export async function CreateNewUser(username: string, data: User): Promise<void> {
    ...
    const email = new GeneralEmail(
        data.email,
        'Bienvenido Sistema de Gestión del Proyecto Docente Integrador',
        welcomeTemplate
    );
